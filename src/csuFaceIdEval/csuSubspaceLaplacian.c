...
Matrix findWMatrix(Matrix Dist, int K, Matrix kMat, int T)
{
	int i, j;
	Matrix W;
	double weight;
	...
	W = makeMatrix (Dist->row_dim, Dist->col_dim);
	kthDistances = makeMatrix(Dist->row_dim, 1);

	for(i = 0; i < Dist->row_dim; i++)
	{
            ME(kthDistances, i, 0) = GetKthItemInRowAfterSorting(Dist, i, K);
	}
	...
	for(i = 0; i < Dist->row_dim; i++)
	{
	    for(j = 0; j < Dist->col_dim; j++)
	    {
            if(ME(Dist, i, j) <= ME(kthDistances, i, 0))
...
{
	int  i, j;
	double sum = 0.0, min = 32000.0, max = 0.0;
...
void laplacianTrain (Matrix imspca, ImageList *srt, Matrix * laplacianBasis, Matrix * laplacianValues, int K, int useAdaptiveK, int lppKeepNVectors, Matrix * pcaValues, char* distName, int writeTextInterm)
{	
	int i, n;
	Matrix Data, Dist, L;
